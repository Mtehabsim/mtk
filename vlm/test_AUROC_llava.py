...
def main(flag):
    ...
    model = AutoModelForCausalLM.from_pretrained("/HARD-DATA/ZHT/A_model/qwen_vl_chat", device_map="cuda",
    ...
    for idx, (dataset_name, dataset) in enumerate(datasets.items(), 1):
        ...
        with open(f"./experimental_results/{flag}/results/test_llava_{dataset_name}.csv", "w", newline="",
                  encoding="utf-8") as f:
            writer = csv.writer(f)
