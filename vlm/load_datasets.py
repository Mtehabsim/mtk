...
def load_vqa_dataset_for_train(json_path="./datasets/vqa/OpenEnded_mscoco_test2015_questions.json",
    ...
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    dataset = [
        [d['question'], os.path.join("./datasets/vqa/test2015", f"COCO_test2015_{str(d['image_id']).zfill(12)}.jpg")]
...
def load_mm_safety_bench(file_path):
    ...
    for i, row in df.iterrows():
        ...
        try:
            ...
            # image.save(f"/HARD-DATA/ZHT/A_dataset/MM-SafetyBench/image/{file_flag}_{i}.png")
            if img_value:
                try:
                    with Image.open(f"/HARD-DATA/ZHT/A_dataset/MM-SafetyBench/image/{file_flag}_{i}.png") as img:
                        img.load()  # 强制加载完整图片数据
                        img.convert("RGB")  # 模拟模型的操作
                except (OSError, Exception) as e:
                    continue
                dataset.append({"txt": row['question'],
                                "img": f"/HARD-DATA/ZHT/A_dataset/MM-SafetyBench/image/{file_flag}_{i}.png",
                                "toxicity": 1})
        except:
            continue
    return dataset
